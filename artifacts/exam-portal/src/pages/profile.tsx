      ...
      <main className="container max-w-xl mx-auto px-6 mt-10">
        <motion.div variants={container} initial="hidden" animate="show" className="space-y-10">
          ...
          <motion.div variants={item} className="flex flex-col items-center text-center">
            <div className="w-24 h-24 rounded-full p-0.5 mb-6" style={{ background: "linear-gradient(135deg, #3b6df0, #7c5bf5)", boxShadow: "0 8px 30px rgba(59,109,240,0.25)" }}>
               <div className="w-full h-full rounded-full flex items-center justify-center overflow-hidden" style={{ background: "#ffffff" }}>
                 {user.photoURL ? (
          ...
          <motion.div variants={item} className="space-y-6">
            ...
            <section className="rounded-3xl p-8 relative overflow-hidden flex flex-col justify-between min-h-[280px]" style={{ background: "linear-gradient(145deg, #ffffff, #f8fafc)", border: "1px solid rgba(59,109,240,0.15)", boxShadow: "0 8px 32px rgba(15,23,42,0.06), 0 0 40px rgba(59,109,240,0.04)" }}>
              <div className="absolute top-0 right-0 w-48 h-48 rounded-full" style={{ background: "radial-gradient(circle, rgba(59,109,240,0.10) 0%, transparent 70%)" }} />
              <div className="relative z-10">
                <div className="w-10 h-10 rounded-xl flex items-center justify-center text-primary mb-4" style={{ background: "rgba(59,109,240,0.10)" }}>
              ...
              <div className="relative z-10 space-y-4">
                ...
                <div className="w-full h-2 rounded-full overflow-hidden" style={{ background: "rgba(15,23,42,0.06)" }}>
                  <div className="h-full rounded-full transition-all duration-700" style={{ width: `${Math.min(100, (stats?.totalAttempts || 0) * 10)}%`, background: "linear-gradient(90deg, #3b6df0, #7c5bf5)" }}></div>
                </div>
              </div>
            </section>
          </motion.div>
             ...
             <div className="space-y-2">
                {submissions && submissions.length > 0 ? (
                   submissions.slice(0, 3).map((sub) => {
                      ...
                      return (
                        <div key={sub.id} onClick={() => setLocation(`/result/${sub.id}`)} className="rounded-2xl p-4 flex items-center gap-4 active:scale-[0.98] transition-transform cursor-pointer" style={{ background: "#ffffff", border: "1px solid rgba(15,23,42,0.07)", boxShadow: "0 1px 3px rgba(15,23,42,0.04)" }}>
                           <div className="w-10 h-10 rounded-xl flex items-center justify-center shrink-0" style={{ background: pct >= 50 ? "rgba(59,109,240,0.10)" : "rgba(239,68,68,0.10)", color: pct >= 50 ? "#3b6df0" : "#dc2626" }}>
                   ...
                   <p className="text-center py-10 text-muted-foreground text-sm rounded-2xl" style={{ border: "1px dashed rgba(15,23,42,0.12)", background: "#ffffff" }}>No recorded sessions yet.</p>
                )}
             </div>
          </motion.div>
          ...
          <motion.div variants={item} className="space-y-2">
             <div className="rounded-2xl p-5 flex items-center gap-4" style={{ background: "#ffffff", border: "1px solid rgba(15,23,42,0.07)", boxShadow: "0 1px 3px rgba(15,23,42,0.04)" }}>
               <div className="w-10 h-10 rounded-lg flex items-center justify-center text-primary shrink-0" style={{ background: "rgba(59,109,240,0.10)" }}>
             ...
             <div className="rounded-2xl p-5 flex items-center gap-4" style={{ background: "#ffffff", border: "1px solid rgba(15,23,42,0.07)", boxShadow: "0 1px 3px rgba(15,23,42,0.04)" }}>
               <div className="w-10 h-10 rounded-lg flex items-center justify-center shrink-0" style={{ background: "rgba(239,68,68,0.10)", color: "#dc2626" }}>
          ...
          <motion.div variants={item} className="pt-4">
            <button className="w-full h-14 rounded-2xl font-bold flex items-center justify-center gap-2 transition-all active:scale-[0.98]" style={{ background: "rgba(239,68,68,0.08)", color: "#dc2626", border: "1px solid rgba(239,68,68,0.18)" }} onClick={handleLogout}>
